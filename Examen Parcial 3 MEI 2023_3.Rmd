...
El modelo asume que las personas cercanas y lejanas a los ríos son tratados a la misma tasa, que la infección cerca o lejos del río sigue el mismo curso, que la reducción en interacción de las personas lejanas es una constante (no decae con la distancia, solo es diferente a partir de un umbral), que la proporción de personas cerca y lejos de los rios permanece constante, no considera la actividad nocturna o diurna de los vectores. Considera Betas distintas de infección a vectores y a humanos, y solo permite tratamiento hasta un umbral.
...
$m(S_V+I_V)$ es la natalidad que compensa la mortalidad de los vectores $\mu$

$m\beta_VS_VI_H$ modela la infección de moscas susceptibles al interactuar con humanos infectados cerca del rio (proporcion m)

$(1-m)\alpha\beta_VS_VI_H$ modela la infección de moscas susceptibles al interactuar con humanos infectados lejos del rio (proporcion 1-m). La interacción entre moscas infectadas y humanos se ve atenuada por $\alpha$

$mt\beta_VS_VI_{HT}$ modela la infección de moscas susceptibles al interactuar con humanos infectados pero también tratados cerca del rio (proporcion m). La interacción no es tan eficaz porque hay menos parasitemia en estos pacientes (atenuado por $t$)

$(1-m)t\alpha\beta_VS_VI_{HT}$ Considera a las personas tratadas y que están lejos del rio (1-m)
...
$(1-m)\alpha\beta_HS_HI_V$ infeccion de personas lejanas al rio 1-m modificada por interacción disminuida ($\alpha$)
...
$(p-I_{HT})I_H$ Tratamiento de personas infectadas hasta alcanzar un valor umbral de tratamiento disponible ($p$)
...
La mayoría de personas y vectores se infecta al final. Al inicio casi todos los infectados se tratan y así no hay un decaimiento rápido de los susceptibles pero se alcanza un punto de saturacion de tratamiento, suben los infectados rápidamente y los susceptibles decaen de igual manera.
...
```{r, cache = TRUE,fig.dim=c(24,5),cache.lazy = FALSE}
library(lubridate)
covid_dates <- datos_covid %>% mutate(num_date=as.numeric(as.POSIXct(FECHA_INGRESO, format="%Y-%m-%d"))) %>% mutate(num_day = num_date/86400)
```
...
hist(filter_dates$num_day,breaks=number_bars,col=rainbow(number_bars),ylab="Numero de casos")
```
- Estima el $R_0$ y $R_t$
